  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex gap-4">
        ...
        <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-extrabold text-slate-950">{practitioner.name}</h3><VerificationBadge /></div><p className="mt-1 font-semibold text-cyan-800">{practitioner.professional_category}</p>{practitioner.specialty && <p className="text-sm text-slate-600">{practitioner.specialty}</p>}</div>
      </div>
      <div className="mt-4 space-y-2 text-sm text-slate-700">
        ...
        <p><strong>Availability:</strong> <span className={practitioner.availability === "available" ? "text-emerald-700" : "text-slate-600"}>{practitioner.availability === "available" ? "Available" : practitioner.availability === "busy" ? "Busy" : "Offline"}</span></p>

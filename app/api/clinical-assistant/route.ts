...
export async function POST(request: Request) {
  ...
  try {
    ...
    const healthInstructions = `\nThe conversation is health-related. Give careful general health information, never a diagnosis or prescription. Ask useful follow-up questions when key context is missing. Do not fabricate patient facts or practitioners. Mention the practitioner directory only when professional care is contextually appropriate. Never claim information was shared. ${isUrgent ? "Put clear urgent-care advice first and tell the user not to wait for an AI reply." : "Identify red flags and recommend professional evaluation when appropriate."} Base condition-specific statements only on the supplied Tanzania STG/NEMLIT 7th Edition (2026) excerpts. If excerpts are insufficient, say so. Format the response with short sections using these exact headings when relevant: URGENT HELP:, IMPORTANT:, and NEXT STEP:. Omit URGENT HELP when there is no urgent warning. For Kiswahili use HUDUMA YA HARAKA:, MUHIMU:, and HATUA INAYOFUATA:. End with exactly: ${SOURCE}`;

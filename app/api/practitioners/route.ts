...
export async function GET() {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("practitioners")
      .select("id,name,professional_category,specialty,registration_authority,masked_registration_number,verification_status,verified_at,verification_expires_at,region,district,availability,image_url,phone,show_public_phone,languages,consultation_hours,updated_at")
    ...
    const { data, error: legacyError } = await supabase.from("practitioners").select("id,name,role,phone,image_url,languages,consultation_hours,is_available,updated_at").order("created_at");
